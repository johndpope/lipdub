...
from models.diffusion_utils import (avg_pool_nd, conv_nd, linear, normalization,
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.dims == 3:
            x = nnf.interpolate(x, (x.shape[2], x.shape[3] * 2, x.shape[4] * 2),
                              mode="nearest")
        else:
            x = nnf.interpolate(x, scale_factor=2, mode="nearest")
        if self.use_conv:
            x = self.conv(x)
        return x
